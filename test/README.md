...
`YanaElfTest` shares the `resolve/` corpus and its `.run.expect` goldens rather than having any of
...
still asserted, in-process, by `YanaResolveTest`. It is skipped entirely off amd64 Linux.

## Sharding

`YanaResolveTest`, `YanaParseTest` and `YanaElfTest` take `shard:i/n` and run the fixtures whose position in the
