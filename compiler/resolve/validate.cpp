...
static bool validateBaseValue(Diagnostics* diagnostics, Value* value) {
    ...
    for(const Use& use: value->uses) {
        ...
        if(!found) {
            diagnostics->error("uses list for value is out of sync"_v, &value->source);
            return false;
        }
    }
...
static bool validateOperand(Diagnostics* diagnostics, Value* source, Value* op) {
    ...
    if(!found) {
        diagnostics->error("instruction has dangling operand"_v, &source->source);
        return false;
    }
    ...
    if(!found) {
        diagnostics->error("uses list for operand is out of sync"_v, &op->source);
        return false;
    }
...
bool validateArg(Diagnostics* diagnostics, Arg* arg) {
    ...
    if(arg->index >= fun->args.size()) {
        diagnostics->error("argument index %@ is out of bounds for function %@"_v, &arg->source, arg->index, fun->name);
        return false;
    }

    if(fun->args[arg->index] != arg) {
        diagnostics->error("argument list for function %@ is out of sync"_v, &arg->source, fun->name);
        return false;
    }
...
bool validateGlobal(Diagnostics* diagnostics, Global* global) {
    ...
    if(global->ast || global->resolving) {
        diagnostics->error("global is not fully resolved"_v, &global->source);
        return false;
    }
...
bool validateInt(Diagnostics* diagnostics, ConstInt* i) {
    ...
    if(i->type->kind != Type::Int) {
        diagnostics->error("integer constant must have integer type"_v, &i->source);
        return false;
    }
    ...
    if(i->value > max) {
        diagnostics->error("integer constant out of range for target type. Constant value: %@, max value: %@"_v, &i->source, i->value, max);
        return false;
    }
...
bool validateFloat(Diagnostics* diagnostics, ConstFloat* i) {
    ...
    if(i->type->kind != Type::Float) {
        diagnostics->error("floating point constant must have float type"_v, &i->source);
        return false;
    }
...
bool validateTrunc(Diagnostics* diagnostics, InstTrunc* i) {
    ...
    if(!valid) {
        diagnostics->error("trunc instructions must truncate from an integer type to a smaller integer type"_v, &i->source);
        return false;
    }
...
bool validateFTrunc(Diagnostics* diagnostics, InstFTrunc* i) {
    ...
    if(!valid) {
        diagnostics->error("ftrunc instructions must truncate from a floating point type to a smaller floating point type"_v, &i->source);
        return false;
    }
...
static bool validateIntExt(Diagnostics* diagnostics, InstCast* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must widen from an integer type to a larger integer type"_v, &i->source, name);
        return false;
    }
...
bool validateFExt(Diagnostics* diagnostics, InstFExt* i) {
    ...
    if(!valid) {
        diagnostics->error("fext instructions must widen from a floating point type to a larger floating point type"_v, &i->source);
        return false;
    }
...
static bool validateFloatToInt(Diagnostics* diagnostics, InstCast* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must convert from a floating point type to an integer type"_v, &i->source, name);
        return false;
    }
...
static bool validateIntToFloat(Diagnostics* diagnostics, InstCast* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must convert from an integer type to a floating point type"_v, &i->source, name);
        return false;
    }
...
static bool validateIntArithmetic(Diagnostics* diagnostics, InstBinary* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must have register-sized integer inputs and output of the same width"_v, &i->source, name);
        return false;
    }
...
static bool validateFloatArithmetic(Diagnostics* diagnostics, InstBinary* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must have register-sized floating point inputs and output of the same width"_v, &i->source, name);
        return false;
    }
...
bool validateICmp(Diagnostics* diagnostics, InstICmp* i) {
    ...
    if(!valid) {
        diagnostics->error("icmp instructions must have register-sized integer inputs of the same width, as well as Bool output"_v, &i->source);
        return false;
    }
...
bool validateFCmp(Diagnostics* diagnostics, InstFCmp* i) {
    ...
    if(!valid) {
        diagnostics->error("fcmp instructions must have register-sized floating point inputs of the same width, as well as Bool output"_v, &i->source);
        return false;
    }
...
static bool validateIntShift(Diagnostics* diagnostics, InstShift* i, const String& name) {
    ...
    if(!valid) {
        diagnostics->error("%@ instructions must have register-sized integer source and output of the same width, as well as a register-sized integer amount"_v, &i->source, name);
        return false;
    }
...
bool validateAddRef(Diagnostics* diagnostics, InstAddRef* i) {
    ...
    if(!valid) {
        diagnostics->error("addref source and resulting type must be untraced references to the same type"_v, &i->source);
        return false;
    }
    ...
    if(!valid) {
        diagnostics->error("addref addition operand must be an integer type of register size"_v, &i->source);
        return false;
    }
...
bool validateBlock(Diagnostics* diagnostics, Block* block) {
    if(!block->complete) {
        diagnostics->error("block is incomplete"_v, &block->function->source);
        return false;
    }
...
static bool validateIntType(Diagnostics* diagnostics, IntType* type, Node* source, Function* fun) {
    ...
    if(type->bits > IntType::bitsForWidth(type->width)) {
        diagnostics->error("integer type is too large for its register width"_v, source);
        return false;
    }

    if(type->bits < 1) {
        diagnostics->error("integer type is too small; it must contain at least 1 bit"_v, source);
        return false;
    }
...
static bool validateFloatType(Diagnostics* diagnostics, FloatType* type, Node* source, Function* fun) {
    ...
    if(type->bits != FloatType::bitsForWidth(type->width)) {
        diagnostics->error("floating point size must be the same as its register width"_v, source);
        return false;
    }
...
static bool validateFunType(Diagnostics* diagnostics, FunType* type, Node* source, Function* fun) {
    for(U32 i = 0; i < type->argCount; i++) {
        ...
        if(arg->index != i) {
            diagnostics->error("function argument list is out of sync"_v, source);
            return false;
        }
...
static bool validateTupType(Diagnostics* diagnostics, TupType* type, Node* source, Function* fun) {
    for(U32 i = 0; i < type->count; i++) {
        ...
        if(field->container != type || field->index != i) {
            diagnostics->error("tuple field list is out of sync"_v, source);
            return false;
        }

        if((field->name != 0) != type->named) {
            diagnostics->error("tuple field naming mode is different from the container naming mode"_v, source);
            return false;
        }
...
static bool validateRecordType(Diagnostics* diagnostics, RecordType* type, Node* source, Function* fun) {
    ...
    if(type->kind == RecordType::Enum) {
        for(U32 i = 0; i < type->conCount; i++) {
            if(type->cons[i].content) {
                diagnostics->error("record type is marked as Enum kind, but constructor %@ contains data"_v, source, i);
                return false;
            }
        }
    } else if(type->kind == RecordType::Single) {
        if(type->conCount != 1) {
            diagnostics->error("record type is marked as Single kind, but number of constructors is %@"_v, source, type->conCount);
            return false;
        }
    } else if(type->kind == RecordType::Multi) {
        if(type->conCount >= 1) {
            diagnostics->error("record type is marked as Multi kind, but number of constructors is %@"_v, source, type->conCount);
            return false;
        }
    } else {
        diagnostics->error("record type has unrecognized kind '%@'"_v, source, (int)type->kind);
        return false;
    }
    ...
    for(U32 i = 0; i < type->conCount; i++) {
        ...
        if(con->parent != type || con->index != i) {
            diagnostics->error("record constructor list is out of sync"_v, source);
            return false;
        }
